...
class Cart {
	...
	public function insert($item = array()){
		if(!is_array($item) OR count($item) === 0){
            ...
            if(!isset($item['item_id'], $item['itemName'], $item['rentPrice'], $item['qty'])){
                return FALSE;
            }else{
                /*
                 * Insert Item
	...
	protected function save_cart(){
		...
		foreach ($this->cart_contents as $key => $val){
			...
			if(!is_array($val) OR !isset($val['rentPrice'], $val['qty'])){
				continue;
			}
	 
			$this->cart_contents['cart_total'] += ($val['rentPrice'] * $val['qty']);
			$this->cart_contents['total_items'] += $val['qty'];
			$this->cart_contents[$key]['subtotal'] = ($this->cart_contents[$key]['rentPrice'] * $this->cart_contents[$key]['qty']);
		}
